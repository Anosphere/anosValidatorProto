...
func main() {
	...
	// Throttle *submission*: 10 goroutines launched per second.
	// IMPORTANT: this does NOT wait for finalization between batches; it only spaces out launch/submission.
	const batchSize = 10
	...
	for idx, s := range senders {
		...
		go func(s sender) {
			...
			// Submit + wait for commit (per sender, but does NOT gate the next batch submission)
		...
		// After every batch of 10 submissions, pause before launching the next batch.
		if (idx+1)%batchSize == 0 && (idx+1) < len(senders) {
